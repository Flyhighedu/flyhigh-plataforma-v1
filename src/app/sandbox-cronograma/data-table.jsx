...
export function DataTable({ columns, data, onUpdateRow, onDeleteRow, onCreateRow }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Native Browser Scrolling Wrapper */}
                    ...
                    <TableHead key={header.id} className="bg-white relative group select-none" style={{ width: header.getSize() }}>
                      ...
                      {header.column.getCanResize() && (
                        <div
                          ...
                          className={`absolute right-0 top-0 h-full w-1 cursor-col-resize select-none touch-none opacity-0 group-hover:opacity-100 transition-opacity ${header.column.getIsResizing() ? "bg-primary opacity-100" : "bg-slate-300"}`}
                        />
                      )}
                    </TableHead>
                  ))}
