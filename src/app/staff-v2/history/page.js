...
import { ArrowLeft, Calendar, ChevronRight, FileText, Search } from 'lucide-react';
...
export default function HistoryPage() {
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            ...
            const numericMissionIds = missionIds
                .map((missionId) => String(missionId))
                .filter((missionId) => /^\d+$/.test(missionId))
                .map((missionId) => Number(missionId));

            const uniqueNumericIds = [...new Set(numericMissionIds)];

            const { data: schools } = uniqueNumericIds.length > 0
                ? await supabase
                    .from('proximas_escuelas')
                    .select('id, nombre_escuela')
                    .in('id', uniqueNumericIds)
                : { data: [] };
            ...
            const schoolMapById = buildSchoolMapById(schools);
            const flightSnapshotMap = buildFlightSnapshotMap(flights);

            const enrichedHistory = (closures || []).map((closure) => {
                const dateTime = missionDateTimeFromClosure(closure);
                const { date, time } = formatDateAndTime(dateTime);

                return {
                    ...closure,
                    schoolName: resolveHistorySchoolName({
                        closure,
                        schoolMapById,
                        flightSnapshotMap
                    }),
