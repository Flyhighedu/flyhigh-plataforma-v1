...
export default function SchoolCombobox({ schools = [], value, onChange, loading = false, disabled = false }) {
    ...
    return (
        <div ref={containerRef} className="relative">
            ...
            {value ? (
                <div className="w-full bg-slate-800/50 border border-amber-500/40 rounded-xl px-4 py-3 flex items-center justify-between gap-3 transition-all">
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                            <School size={14} className="text-amber-400 shrink-0" />
                            <span className="font-bold text-white truncate">{value.nombre_escuela}</span>
                        </div>
                    ...
                    {!disabled && (
                        <button
                            type="button"
                            onClick={handleClear}
                            className="p-1.5 hover:bg-red-500/20 rounded-lg transition-colors text-slate-400 hover:text-red-400 shrink-0"
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-slate-800/50 border border-slate-700 rounded-xl pl-11 pr-10 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all disabled:opacity-50"
                    />
            ...
            {/* Dropdown */}
            {isOpen && !value && (
                <div className="absolute z-50 mt-2 w-full bg-slate-900/95 backdrop-blur-xl border border-slate-700/80 rounded-xl shadow-2xl shadow-black/40 overflow-hidden">
                    {loading ? (
                        ...
                        <div className="py-6 text-center text-slate-400">
                            <School size={24} className="mx-auto mb-2 opacity-40" />
                            <p className="text-sm">No se encontraron escuelas</p>
                            <p className="text-[10px] text-slate-500 mt-1">Intenta con otro nombre o CCT</p>
                        </div>
                    ) : (
                        <>
                            ...
                            >
                                {filtered.map((school, idx) => (
                                    <li
                                        ...
                                        className={`px-3 py-2.5 cursor-pointer border-b border-slate-800/50 last:border-b-0 transition-colors ${
                                            idx === highlightIndex
                                                ? 'bg-amber-500/15'
